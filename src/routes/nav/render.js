...
const RENDERER = {
    ...
    render : function(){
        ...
        let rate = this.easeInOutQuad(Math.min(1, this.statusCount / this.MAX_STATUS_COUNT));
...
let CARD = function(renderer, row, column){
...
CARD.prototype = {
    ...
    init : function(){
        let index = this.column * this.renderer.ROW + this.row;
    ...
    controlStatus : function(context, status, rate){
        if(status === 0){
            ...
            let next = (status === this.renderer.MAX_STATUS) ? 0 : status;
