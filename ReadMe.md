...
- **状态**（`recommend` 三态，编辑器下拉选择）：
  ...
  - `1` 推荐（列表/详情带 ⭐ 标记，排序靠前）
  - `-1` 加密（仅管理员登录后可见正文）
- **加密访问**：未登录时列表仅显示标题与 🔒 图标，正文接口返回 403；登录管理员账号后默认可见全部加密笔记（登录即解锁）。父级加密则整棵子树继承加锁。

## 内容格式与编辑

正文**统一以 Markdown 存储**（允许内嵌 HTML，由 `rehype-raw` + `rehype-sanitize` 安全渲染）。编辑器两种模式：

- **文档**：左 Markdown 源码、右实时预览，与详情页同一套渲染（所见即所得）；粘贴/拖拽图片自动上传插入 `![](url)`。
- **可运行应用**：正文为单个 ` ```live ` 代码块时启用，块内 HTML/JS 放进 **sandbox iframe** 执行（隔离主站，无法访问登录凭证），右侧实时预览。用于贷款计算器、行情面板等小工具。

## 安全

- `helmet` 安全响应头（按需放行内嵌 HTML / data 图 / 沙箱 iframe）。
- 登录限流：单 IP 15 分钟最多 10 次，防管理员密码暴力破解。
- 加密内容遮罩在服务端完成（列表抹除摘要/标签、详情 403），不依赖前端。
